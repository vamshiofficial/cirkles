...
export default function TermsConditionsPage({navigation}) {
  const [loading, SetLoading] = useState(true);
  const [Data, SetData] = useState('');
  const [DataAvailable, SetDataAvailable] = useState(false);

  useEffect(() => {
    const apiURL = `https://esigm.com/thecircle/v1/rules.php?action=get_terms_and_conditions_page_content`;
    fetch(apiURL)
      .then(res => res.json())
      .then(resJson => {
        SetData(resJson);
